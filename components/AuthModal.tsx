      ...
      <div style={{
        ...
        <div style={{
          ...
          {mode === "forgot" ? (
            ...
            <>
            <GoogleButton onClick={signInWithGoogle} busy={googleBusy} compact={isMobile} />
            <OrDivider />
            <form onSubmit={submit} style={{ flex: 1, display: "flex", flexDirection: "column", gap: "14px" }}>
              <div>
                <FieldLabel label="EMAIL" />
...
function GoogleButton({ onClick, busy, compact }: { onClick: () => void; busy: boolean; compact?: boolean }) {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={busy}
      style={{
        width: "100%", height: compact ? "42px" : "48px", borderRadius: "12px",
      ...
      onMouseEnter={e => { if (!busy) e.currentTarget.style.background = "rgba(255,255,255,0.1)"; }}
...
function OrDivider() {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "12px", margin: "14px 0" }}>
      ...
      <span style={{ fontSize: "10px", letterSpacing: "0.1em", color: "rgba(255,255,255,0.3)" }}>OR</span>
      <div style={{ flex: 1, height: "1px", background: "rgba(255,255,255,0.08)" }} />
    </div>
  );
}
